...
iptables -A FORWARD -i eth0 -o link0 -p tcp --syn --dport 8080 -m conntrack --ctstate NEW -j ACCEPT
iptables -A FORWARD -i eth0 -o link0 -p tcp --syn --dport 8443 -m conntrack --ctstate NEW -j ACCEPT
...
iptables -t nat -A PREROUTING -i eth0 -p tcp --dport 8080 -j DNAT --to-destination 10.0.0.2:8080
iptables -t nat -A PREROUTING -i eth0 -p tcp --dport 8443 -j DNAT --to-destination 10.0.0.2:8443
...
# generic udp proxies
socat UDP4-RECVFROM:19522,fork UDP4-SENDTO:10.0.0.2:19522,sp=19524,reuseaddr &
socat UDP4-RECVFROM:19523,fork UDP4-SENDTO:10.0.0.2:19522,sp=19525,reuseaddr
